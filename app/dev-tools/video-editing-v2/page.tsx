...
/**
 * Manual Video Editor V2 (isolated rebuild) — /dev-tools/video-editing-v2
 *
 * Forked from /tools/video-editing (do NOT edit that file from here, and
 * do not import from it — this route is intentionally standalone until the
 * new model is fully tested, per the isolation requirement for this pass).
 *
 * WHAT CHANGED FROM V1: the single-video, single-audio-file model is
 * replaced with a real layered-track model, closer to a standard NLE:
 *   MUSIC   (top)    — multiple independent music/audio clips
 *   TEXT              — unchanged from v1 (multiple timed text overlays)
 ...
 *
 * Video and Music clips reuse the same flex-grow "block" timeline mechanics
 * v1 built (split/trim/drag-reorder/lock/duplicate/delete, all unchanged),
 * generalized from a single shared source file to one file per clip (see
 * `ClipBase`). Overlay reuses v1's Text-track mechanics (absolute time-based
  ...
  return (
    ...
    <div style={{ height: "100vh", width: "100%", display: "flex", flexDirection: "column", backgroundColor: COLORS.bg, color: COLORS.textPrimary, overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "10px 20px", borderBottom: `1px solid ${COLORS.cardBorder}`, backgroundColor: COLORS.trackHeaderBg, flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.05em", padding: "3px 6px", borderRadius: 4, border: `1px solid ${COLORS.accent}`, color: COLORS.accent }}>
          V2 PROTOTYPE — MULTI-TRACK
        </span>
